...
//   RequestParameters - Structure, Map - параметры, которые будут отправлены в URL (часть после ?).
//                                                См. описание Session.RequestParameters.
//   AdditionalParameters - Structure - см. описание параметра в CallMethod.
//   Session - Structure - см. возвращаемое значение функции NewSession.
//
// Возвращаемое значение:
//   Structure - ответ на выполненный запрос. См. описание возвращаемого значения в CallMethod.
//
...
// Параметры:
//   URL - String - URL ресурса, к которому будет отправлен запрос.
//   Data - Structure, Map, String, BinaryData - см. описание AdditionalParameters.Data.
//   AdditionalParameters - Structure - см. описание параметра в CallMethod.
//   Session - Structure - см. возвращаемое значение функции NewSession.
//
// Возвращаемое значение:
...
//                             Параметры преобразования см. AdditionalParameters.JSONConversionParameters.
//
...
//
// Параметры:
//   Method - String - имя HTTP-метода для запроса.
//   URL - String - URL ресурса, к которому будет отправлен запрос.
//   AdditionalParameters - Structure - позволяет задать дополнительные параметры:
//      *Заголовки - Map - см. описание Session.Headers.
//      *Аутентификация - Structure - см. описание Session.Authentication
//      *Прокси - InternetProxy - см. описание Session.Proxy.
//      *ПараметрыЗапроса - Structure, Map - см. описание Session.RequestParameters.
//      *ПроверятьSSL - Булево - см. описание Session.VerifySSL.
//      *КлиентскийСертификатSSL - см. описание Session.ClientSSLCertificate.
//      *Cookies - Array - см. описание Session.Cookies.
//      *Таймаут - Number - время ожидания осуществляемого соединения и операций, в секундах.
//                 Value по умолчанию - 30 сек.
//      *РазрешитьПеренаправление - Булево - True - редиректы будут автоматически разрешены.
...
//          **ПрочитатьВСоответствие - Булево - If True, чтение объекта JSON будет выполнено в Map.
//                                              If False, объекты будут считываться в объект типа Structure.
//          **ФорматДатыJSON - JSONDateFormat - формат, в котором представлена дата в строке,
//                             подлежащей преобразованию.
//          **ИменаСвойствСоЗначениямиДата -  Array, String - имена свойств JSON,
//                                            для которых нужно вызывать восстановление даты из строки.
//      *ПараметрыЗаписиJSON - Structure - параметры, используемые при записи объекта JSON.
//                                         См. в синтаксис-помощнике описание JSONWriterSettings.
//      *Данные - Structure, Map - поля формы, которые необходимо отправить в запрос:
//          **<Key> - String - имя поля.
//          **<Value> - String - значение поля.
//              - String, BinaryData - произвольные данные, которые необходимо отправить в запросе.
//      *Файлы - Structure, Array - файлы, которые необходимо отправить в запросе:
//          **Имя - String - имя поля формы.
//          **Данные - BinaryData - двоичные данные файла.
//          **ИмяФайла - String - имя файла.
//          **Тип - String - MIME-тип файла.
//          **Заголовки - Map, Undefined - HTTP заголовки запроса.
//      *МаксимальноеКоличествоПовторов - Number - количество повторных попыток соединения/отправки запроса.
...
//      *МаксимальноеВремяПовторов - Number - максимальное общее время (в секундах) отправки запроса с учетом повторов.
//                                   Value по умолчанию: 600.
//      *КоэффициентЭкспоненциальнойЗадержки - Number - коэффициент изменения экспоненциальной задержки.
...
//      *ПовторятьДляКодовСостояний - Undefined - повторы будут выполняться для кодов состояний >= 500.
...
//      *Заголовки - Map - HTTP заголовки ответа.
//      *ЭтоПостоянныйРедирект - Булево - признак постоянного редиректа.
//      *ЭтоРедирект - Булево - признак редиректа.
//      *Кодировка - String - кодировка текста ответа.
//      *Тело - BinaryData - тело ответа.
//      *КодСостояния - Number - код состояния ответа.
...
//      *Заголовки - Map - HTTP заголовки запроса.
...
//                                                   Value по умолчанию: False.
//          *Тип - String - тип аутентификации. For Basic Type можно не указывать.
//       If Type = Digest или Basic:
//          *Пользователь - String - имя пользователя.
//          *Пароль - String - пароль пользователя.
//       If Type = AWS4-HMAC-SHA256:
//          *ИдентификаторКлючаДоступа - String - идентификатор ключа доступа.
//          *СекретныйКлюч - String - секретный ключ.
//          *Сервис - String - сервис, к которому выполняется подключение.
//          *Регион - String - регион, к которому выполняется подключение.
//      *Прокси - InternetProxy - параметры прокси, которые будут использованы при отправке запроса.
//                Value по умолчанию: Undefined. При этом если в конфигурации используется БСП,
//                                                     то значения прокси будет взято из БСП.
//      *ПараметрыЗапроса - Structure, Map - параметры, которые будут отправлены в URL (часть после ?):
//          *<Key> - String - ключ параметра в URL.
//          *<Value> - String - значение параметра URL
//                      - Array - сформирует строку из нескольких параметров: key=value1&key=value2 и т.д.
//      *ПроверятьSSL - Булево - False - проверка сертификата сервера не выполняется.
//                             - True - используется значение OSCertificationAuthorityCertificates.
//                    - FileCertificationAuthorityCertificates - см. в синтаксис-помощнике описание
//                                                             FileCertificationAuthorityCertificates.
//                      Value по умолчанию: True.
//      *КлиентскийСертификатSSL - FileClientCertificate - см. в синтаксис-помощнике описание FileClientCertificate.
//                               - WindowsClientCertificate - см. в синтаксис-помощнике описание
...
//                                               Value по умолчанию:
//                                               см. функцию MaximumNumberOfRedirects
...
//
// Параметры:
//   Response - Structure - ответ сервера на отправленный запрос.
//                      См. описание возвращаемого значения CallMethod.
//   JSONConversionParameters - Structure - задает параметры преобразования JSON.
//      *ПрочитатьВСоответствие - Булево - If True, чтение объекта JSON будет выполнено в Map.
//                                         If False, объекты будут считываться в объект типа Structure.
//      *ФорматДатыJSON - JSONDateFormat - формат, в котором представлена дата в строке, подлежащей преобразованию.
//      *ИменаСвойствСоЗначениямиДата -  Array, String - имена свойств JSON,
//                                       для которых нужно вызывать восстановление даты из строки.
//
// Возвращаемое значение:
//   Map - ответ сервера в виде десериализованного значения JSON.
//                  If ConversionParameters.ReadToMap = True (по умолчанию).
//   Structure - если ConversionParameters.ReadToMap = False.
//
...
//   Response - Structure - ответ сервера на отправленный запрос.
//                       См. описание возвращаемого значения CallMethod.
//   Encoding - String, TextEncoding - определяет кодировку текста.
//                                         If значение не задано, то кодировка извлекается из Response.Encoding.
//
// Возвращаемое значение:
...
//   Response - Structure - ответ сервера на отправленный запрос.
//                       См. описание возвращаемого значения CallMethod.
//   XMLReaderSettings - XMLReaderSettings - Параметры чтения, которые будут использованы при чтении данных XML
//                                             Подробнее см. в синтакс помощнике метод XMLReader.OpenStream
//   XMLSchemaSet - XMLSchemaSet - Набор схем XML, используемых при проверке читаемого документа XML.
//                                 If набор схем указан, но не проверен и включена проверка документа XML,
//                                 то будет выполнена проверка набора схем
//                                 Подробнее см. в синтакс помощнике метод XMLReader.OpenStream
//   Encoding - String, TextEncoding - Позволяет задать кодировку.
//                                         Подробнее см. в синтакс помощнике метод XMLReader.OpenStream
//
// Возвращаемое значение:
//   XDTOObject - XDTOObject, СписокXDTO - тип возвращаемого значения может быть любым
...
//      *Схема - String - схема обращения к серверу (http, https).
//      *Аутентификация - Structure - параметры аутентификации:
//          *Пользователь - String - имя пользователя.
//          *Пароль - String - пароль пользователя.
//      *Сервер - String - адрес сервера.
//      *Порт - Number - порт сервера.
//      *Путь - String - адрес ресурса на сервере.
//      *ПараметрыЗапроса - Map - параметры запроса передаваемые на сервер в URL (часть после ?):
//          *<Key> - String - ключ параметра в URL.
//          *<Value> - String - значение параметра URL;
//                      - Array - значения параметра (key=value1&key=value2).
//      *Фрагмент - String - часть URL после #.
//
...
//
// Параметры:
//   Object - Произвольный - данные, которые необходимо преобразовать в JSON.
//   ConversionParameters - Structure - кодировка текста JSON. Value по умолчанию - utf-8.
//      *ФорматДатыJSON - JSONDateFormat - определяет формат сериализации дат JSON-объектов.
//      *ВариантЗаписиДатыJSON - JSONDateWritingVariant - определяет вариант записи даты в формате JSON.
//      *ИмяФункцииПреобразования - String - функция, которая вызывается для всех свойств,
//                                  тип которых не поддерживает автоматическую сериализацию в JSON.
//                                  Function должна быть экспортируемая и иметь следующие параметры:
//                                    **Свойство - String - свойство структуры данных, которое не может быть
//                                                          автоматически сериализовано в JSON.
//                                    **Значение - String - значение свойства структуры данных, которое
...
//      *МодульФункцииПреобразования - Произвольный - модуль, в котором определена функция ConvertionFunctionName.
//      *ДополнительныеПараметрыФункцииПреобразования - Произвольный - параметры, которые будут переданы
//                                                                     в функцию ConvertionFunctionName.
//   WriterSettings - Structure - параметры преобразования JSON:
...
// Преобразование JSON в Object.
//
// Параметры:
//   Json - Поток, BinaryData, String - данные в формате JSON.
//   Encoding - String - кодировка текста JSON. Value по умолчанию - utf-8.
//   ConversionParameters - Structure - параметры преобразования JSON:
//      *ПрочитатьВСоответствие - Булево - если True, чтение объекта JSON будет выполнено в Map,
//                                         иначе в Structure.
//      *ИменаСвойствСоЗначениямиДата - Array, String, ФиксированныйМассив - имена свойств JSON,
//                                      для которых нужно вызывать восстановление даты из строки.
